...
test_that("Fields Book - Chapter 10 results match", {
  ...
  tableOutput1 <- results[["results"]][["factorDescriptives"]][["data"]]
  expect_equal_tables(tableOutput1,
                      list(0, 178, "TRUE",
                           1, 222, "FALSE")
  )
  tableOutput2 <- results[["results"]][["modelSummary"]][["data"]]
  ...
  tableOutput3 <- results[["results"]][["estimatesTable"]][["data"]]
  ...
  tableOutput6 <- results[["results"]][["estimatesTable"]][["data"]]
  ...
  tableOutput7 <- results[["results"]][["estimatesTable"]][["data"]]
  ...
  results <- jasptools::run("RegressionLogistic", dataset = rio::import("~/Dropbox/ej_andy_shared/spss_tutorials/spss_glm_10/www/santas_log.sav"), options, view=FALSE, quiet=TRUE)
  tableOutput9 <- results[["results"]][["casewiseDiagnosticsTable"]][["data"]]
  ...
  tableOutput10 <- results[["results"]][["estimatesTableBootstrapping"]][["data"]]
