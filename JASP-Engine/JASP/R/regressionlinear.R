...
RegressionLinear <- function(dataset=NULL, options, perform="run", callback=function(...) 0, ...) {
	...
	if (options$regressionCoefficientsEstimates == TRUE && options$regressionCoefficientsBootstrapping == TRUE) {
	  ...
	  fields <- list(
	    ...
	    list(name = "Standard Error", type="number", format = "sf:4;dp:3"))
	    #list(name = "p", title = "One-sided p\u002A", type = "number", format = "dp:3;p:.001"))
	    # p-value left out for now due as it is technically not a p-value but the % of bootstraps in the opposite direction than the coefficient
	  
	  #.addFootnote(footnotes, symbol = "\u002A", text = "The p-value provided is technically not a p-value, but indicates the % of bootstraps with the opposite sign than the unstandardized coefficient.")
	    ...
	    for (m in 1:length(lm.model)) {
	      ...
	      if ( class(lm.model[[ m ]]$lm.fit) == "lm" && (! (length(lm.model[[m]]$variables) == 0 && options$includeConstant == FALSE))) {
	        ...
	        if (options$method != "enter") {
	          model.formula <- paste0(dependent.base64, " ~ ")
	          if (options$includeConstant == TRUE) {
	            model.formula <- paste0(model.formula, "1")
	          } else {
	            model.formula <- paste0(model.formula, "0")
	          }
	          if (! is.null(lm.model[[m]]$variables)) {
	            for (variable in lm.model[[m]]$variables) {
	              model.formula <- paste0(model.formula, " + ", .v(variable))
	            }
	          }
	        }
	        ...
	        bootstrap.summary <- boot::boot(data = dataset, statistic = .bootstrapping, R = options$regressionCoefficientsBootstrappingReplicates, formula = model.formula, wlsWeights = .v(options$wlsWeights))
	        bootstrap.coef <- bootstrap.summary$t0
	        bootstrap.bias <- apply(bootstrap.summary$t, 2, mean) - bootstrap.coef
	        bootstrap.se <- apply(bootstrap.summary$t, 2, sd)
	        bootstrap.pvalue <- 1 - sign(bootstrap.summary$t0) * apply(sign(bootstrap.summary$t), 2, mean)
	        ...
	        if (options$includeConstant == TRUE) {
	          ...
	          if(is.null(na.estimate.names) || na.estimate.names[1] != "(Intercept)"){
	            ...
	            bootstrap.regression.result[[ len.reg ]]$"p" <- as.numeric(bootstrap.pvalue[v])
	        ...
	        if (length(lm.model[[ m ]]$variables) > 0) {
	          ...
	          for (var in 1:length(variables.in.model)) {
	            ...
	            if (!is.null(na.estimate.names) && .v(variables.in.model[var])%in%na.estimate.names) {
	              ...
	              bootstrap.regression.result[[ len.reg ]]$"p" <- as.numeric(bootstrap.pvalue[v+var])
		...
		if (perform == "run" && length(list.of.errors) == 0 && dependent.variable != "") {
			...
			if (is.null(lm.fit) || length(options$modelTerms) == 0) {
			 ...
			 casewiseDiagnostics.rows[[length(casewiseDiagnostics.rows)+1]] <- list(caseNumber=".", stdResidual=".", dependentVariable=".", predictedValue=".", residual=".", cooksD = ".")
			  ...
			  casewiseDiag <- .casewiseDiagnostics(lm.fit, diagnosticsType=options$residualsCasewiseDiagnosticsType, outliersOutside=options$residualsCasewiseDiagnosticsOutliersOutside, cooksDistance=options$residualsCasewiseDiagnosticsCooksDistance)
