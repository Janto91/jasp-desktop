...
.casewiseDiagnosticsTable <- function(dataset, model, options) {
  ...
  fields <- list(
    ...
    list(name = "dependentVariable", title = options$dependent, type="string", format = "sf:4;dp:3"),
  ...
  if (perform == "run") {
    ...
    if (is.null(model)) {
      ...
      casewiseDiagnostics.rows[[length(casewiseDiagnostics.rows)+1]] <- list(caseNumber=".", dependentVariable=".", cooksD = ".")
      
    } else {
      
      casewiseDiag <- .casewiseDiagnostics(dataset, model, options)
...
.casewiseDiagnostics <- function(dataset, model, options) {
  ...
  if (options$casewiseDiagnosticsType == "cooksDistance") {
  ...
  if (length(index) == 0) {
    ...
    dependent <- dataset[[.v(options$dependent)]][index]
